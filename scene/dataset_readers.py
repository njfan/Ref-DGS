...
class SceneInfo(NamedTuple):
    ...
    ref_point_cloud: BasicPointCloud
    train_cameras: list
    test_cameras: list
...
def readNerfSyntheticInfo(path, white_background, eval, extension=".png"):
    ...
    ref_num_pts = 50_000
    ref_xyz = np.random.random((ref_num_pts, 3)) * 2 - 1
    ref_shs = np.random.random((ref_num_pts, 3)) / 255.0
    ref_pcd = BasicPointCloud(points=ref_xyz, colors=SH2RGB(ref_shs), normals=np.zeros((ref_num_pts, 3)))
